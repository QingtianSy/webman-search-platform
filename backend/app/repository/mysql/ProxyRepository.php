...
class ProxyRepository
{
    ...
    // 以下 *Strict 方法用于后台 CRUD：DB 故障抛 RuntimeException，避免把基础设施故障伪装成
    // "代理不存在/创建失败/空列表"。运行期链路（findByLocation/markUsed）仍用非严格版本。

    public function createStrict(array $data): int
    {
        $pdo = MySqlClient::pdo();
        if (!$pdo) {
        ...
        try {
            ...
            throw new \RuntimeException('proxy create failed: ' . $e->getMessage(), 0, $e);
        }
    }

    public function updateStrict(int $id, array $data): bool
    {
        $pdo = MySqlClient::pdo();
        if (!$pdo) {
            throw new \RuntimeException('MySQL connection unavailable');
        ...
        try {
            ...
            if (empty($sets)) {
                // 无有效字段不算 DB 故障；service 层据此返回"无字段可更新"。
                return false;
            }
            $sets[] = 'updated_at = NOW()';
            $sql = 'UPDATE proxies SET ' . implode(', ', $sets) . ' WHERE id = :id';
            ...
            throw new \RuntimeException('proxy update failed: ' . $e->getMessage(), 0, $e);
        }
    }
    ...
    public function deleteStrict(int $id): bool
    {
        ...
        try {
            ...
            throw new \RuntimeException('proxy delete failed: ' . $e->getMessage(), 0, $e);
        }
    }
    ...
    public function findByIdStrict(int $id): array
    {
        ...
        try {
            ...
            throw new \RuntimeException('proxy find failed: ' . $e->getMessage(), 0, $e);
        }
    }

    public function listStrict(int $page = 1, int $pageSize = 20, array $filters = []): array
    {
        $pdo = MySqlClient::pdo();
        if (!$pdo) {
            throw new \RuntimeException('MySQL connection unavailable');
        ...
        try {
            ...
            throw new \RuntimeException('proxy list failed: ' . $e->getMessage(), 0, $e);
        }
    }

    public function batchCreateStrict(array $items): int
    {
        $pdo = MySqlClient::pdo();
        if (!$pdo) {
            throw new \RuntimeException('MySQL connection unavailable');
        ...
        try {
            ...
            throw new \RuntimeException('proxy batchCreate failed: ' . $e->getMessage(), 0, $e);
        }
    }
    ...
    public function allStrict(): array
    {
        ...
        try {
            ...
            throw new \RuntimeException('proxy all failed: ' . $e->getMessage(), 0, $e);
        }
    }
}
