...
class OperateLogRepository
{
    public function listByUserId(int $userId, int $page = 1, int $pageSize = 20): array
    {
        ...
        try {
            ...
            $stmt = $pdo->prepare('SELECT id, user_id, module, action, content, ip, created_at FROM operate_logs WHERE user_id = :user_id ORDER BY created_at DESC LIMIT :limit OFFSET :offset');
    ...
    // 严格版本：DB 故障抛 RuntimeException，避免操作记录接口用"空列表"掩盖故障。
    public function listByUserIdStrict(int $userId, int $page = 1, int $pageSize = 20): array
    {
        ...
        try {
            ...
            throw new \RuntimeException('operate log list failed: ' . $e->getMessage(), 0, $e);
        }
    }
