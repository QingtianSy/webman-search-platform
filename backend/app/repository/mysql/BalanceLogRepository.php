...
class BalanceLogRepository
{
    public function listByUserId(int $userId, int $page = 1, int $pageSize = 20): array
    {
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT id, user_id, type, amount, balance_after, remark, created_at FROM balance_logs WHERE user_id = :user_id ORDER BY created_at DESC LIMIT {$pageSize} OFFSET {$offset}");
